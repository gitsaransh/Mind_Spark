...
const HintSystem = {
    ...
    generateHint(puzzle, hintLevel) {
        // v2.1: Use specific hint if available (for first hint)
        if (puzzle.hint && hintLevel === 1) {
            return puzzle.hint;
        }
        const hints = {
            // Pattern puzzles
            ...
            // Logic puzzles
            logic: [
                "Read the question carefully and break it down into smaller parts.",
                "Try to eliminate the impossible options first."
            ],

            // Math puzzles
            math: [
                "Look for the underlying mathematical relationship.",
                "Try working backwards from the options if you're stuck."
            ]
        };
